...
bot.on('message', async (msg) => {
  ...
  if (adminSession.step === 'survey_add_question_text') {
    ...
    if ((adminSession.data.questionType || '').toLowerCase() === 'text') {
      await bot.sendMessage(chatId, 'This is a text survey. Type SKIP to continue without options.');
    } else {
      await bot.sendMessage(chatId, 'Enter answer options separated by | (example: A|B|C|D):');
    }
    return;
  }
  ...
  if (adminSession.step === 'survey_add_question_options') {
    ...
    if (questionType === 'text') {
      if (raw.toUpperCase() !== 'SKIP') {
        await bot.sendMessage(chatId, 'For text surveys, type SKIP to continue.');
        return;
      }
