...
async function generateShareholderIdForUser(userName) {
  ...
  for (const row of result.rows) {
    const id = row.shareholder_id || '';
    let match = id.match(new RegExp(`^SHA-${year}-${initials}(\\d+)-${SHAREHOLDER_ID_SUFFIX}$`));

    // Backward compatibility with older generated IDs: SHA-YYYY-INITIALS-01-SUFFIX
    if (!match) {
      match = id.match(new RegExp(`^SHA-${year}-${initials}-(\\d+)-${SHAREHOLDER_ID_SUFFIX}$`));
    }
  ...
  if (isLoggedIn) {
    ...
    if (user) {
      ...
      const welcomeMessage = `👋 Welcome back, ${user.name}!\n\n` +
                            ...
                            `/sh_help - Shareholder command guide\n` +
...
`;
  message += `• /shareholders - Open shareholder dashboard
`;
  message += `• /sh_topup - Start shareholder stake top-up (USD)
`;
  message += `• /sh_withdraw - Request shareholder earnings withdrawal

`;

  message += `🧭 **User Flow (Top-Up)**
`;
  message += `1) /sh_topup
`;
  message += `2) Enter amount in USD (minimum applies)
`;
  message += `3) Enter payment method
`;
  message += `4) Enter transaction reference or SKIP
`;
  message += `5) Optional: upload screenshot/document proof
`;
  message += `6) Wait for admin approval

`;

  message += `🧭 **User Flow (Withdraw Earnings)**
`;
  message += `1) /sh_withdraw
`;
  message += `2) If lock period (6 months) is complete, enter amount
`;
  message += `3) Enter payout method
`;
  message += `4) Enter payout details
`;
  message += `5) Wait for admin approval
...
`;
    message += `• /sh_create USER_ID
`;
    message += `• /sh_view USER_ID_OR_SHA_ID
`;
    message += `• /sh_adjust USER_ID_OR_SHA_ID AMOUNT REASON
`;
    message += `• /sh_setstatus USER_ID_OR_SHA_ID active|suspended|under_review
`;
    message += `• /sh_suspend USER_ID_OR_SHA_ID REASON
`;
    message += `• /sh_unsuspend USER_ID_OR_SHA_ID REASON
`;
    message += `• /sh_delete USER_ID_OR_SHA_ID REASON
`;
    message += `• /sh_list [QUERY]
`;
    message += `• /sh_pending
`;
    message += `• /sh_approve_topup REQ_ID
`;
    message += `• /sh_reject_topup REQ_ID REASON
`;
    message += `• /sh_approve_withdraw REQ_ID
`;
    message += `• /sh_reject_withdraw REQ_ID REASON

`;

    message += `✅ **Examples**
`;
    message += `/sh_create USER-1001
`;
    message += `/sh_view SHA-2026-BM01-UI
`;
    message += `/sh_adjust USER-1001 250 Stake correction
`;
    message += `/sh_setstatus USER-1001 active
`;
    message += `/sh_approve_topup SHT-1730000000000
`;
    message += `/sh_reject_withdraw SHW-1730000000000 Invalid details`;
  } else {
    message += `ℹ️ Need admin actions? Contact support/admin.`;
  }
...
bot.onText(/\/sh_create (.+)/, async (msg, match) => {
  ...
  if (memberId.startsWith('SHA-')) {
    await bot.sendMessage(chatId,
      `❌ /sh_create expects a USER member ID, not a shareholder ID.
` +
      `Example: /sh_create USER-1001
` +
      `Use /sh_view ${memberId} to view an existing shareholder profile.`
    );
    return;
  }
